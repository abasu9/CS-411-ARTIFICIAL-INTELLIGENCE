...
15-Puzzle Solver Using Iterative Deepening Depth First Search (IDDFS)
Overview
Welcome to the 15-Puzzle Solver! This program implements the Iterative Deepening Depth First Search (IDDFS) algorithm to solve the classic 15-puzzle game. The objective is to rearrange numbered tiles in ascending order, with the empty space represented by 0 located in the bottom-right corner.
Key Features
* Efficient Search: Combines the benefits of depth-first and breadth-first search to minimize memory usage while exploring possible configurations.
* Comprehensive Reporting: Outputs detailed results, including:
o The sequence of moves to achieve the solution.
o The total number of nodes expanded during the search.
o Memory usage statistics.
o Execution time for solving the puzzle.
Requirements
* Python 3.x: Ensure you have Python 3 installed on your machine.
* Standard Libraries: The program uses built-in Python libraries, so no additional installations are required.
Getting Started
Steps to Run the Program
1. Download the Project Files: Ensure you have abasu9_iddfs.py and this README.txt.
2. Open Your Terminal or Command Prompt.
3. Navigate to the Project Directory: Use the cd command to change to the folder containing iddfs.py.
4. Run the Program: Enter the following command:
5. Input the Puzzle Configuration: When prompted, enter the initial arrangement of the puzzle as 16 space-separated numbers. Remember, the number 0 represents the empty tile.
Example Input:
1 0 2 4 5 7 3 8 9 6 11 12 13 10 14 15
6. View the Results: The program will output:
o Moves: The directions needed to solve the puzzle (L = Left, R = Right, U = Up, D = Down).
o Nodes Expanded: The number of unique configurations explored during the search.
o Memory Used: The maximum memory utilized is measured in kilobytes.
o Time Taken: The duration required to find the solution is measured in seconds.
OUTPUT :



Additional Notes
* Ensure the initial configuration is valid (exactly 16 numbers, including 0).
* The time taken to solve the puzzle may vary based on its complexity; simpler puzzles will be solved more quickly, while complex configurations may take longer.
SOURCE CODE :
